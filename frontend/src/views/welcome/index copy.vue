...
<template>
  <el-form :model="data.form" label-width="120px" ref="formRef" class="el-form" :rules="rules">
    <el-tabs v-model="activeName" class="demo-tabs" type="border-card" @tab-click="handleClick">
      <el-tab-pane label="基础配置" name="first">
        ...
        <el-form-item label="进入直播间消息" prop="EntryMsg">
      ...
      <el-tab-pane label="欢迎弹幕自定义" name="second">
        ...
        <el-form-item v-if="data.form.InteractWordByTime == false" v-for="(items, index) in data.form.WelcomeDanmu"
          :label="'欢迎弹幕 ' + (index + 1)">
          <el-input v-model="data.form.WelcomeDanmu[index]" />
          <el-button class="mt-2" @click="deleteWelcomeDanmu(index)">删除
          </el-button>
        </el-form-item>
    ...
    getWelcomeDanmuByTimeDescribe(
      data.form.WelcomeDanmuByTime[index].Key
          ...
          <el-form-item v-for="(items, index2) in data.form.WelcomeDanmuByTime[index].Danmu"
            :label="'欢迎弹幕 ' + (index2 + 1)">
            <el-input v-model="data.form.WelcomeDanmuByTime[index].Danmu[index2]" />
            <el-button class="mt-2" @click="deleteWelcomeDanmuByTime(index, index2)">删除
            </el-button>
          </el-form-item>
        </div>
      </el-tab-pane>
        ...
        <el-table :data="data.KeywordReplyTemp" border>
          ...
          <el-table-column label="操作">
            <template #default="scope">
              <el-button type="primary" @click="editKeyword(scope.row.key, scope.row.value)">编辑
              </el-button>
              <el-button type="danger" @click="deleteKeyword(scope.row.key)">删除
              </el-button>
            </template>
          </el-table-column>
      ...
      <el-tab-pane label="关注答谢语" name="fifth">
        ...
        <el-form-item v-for="(items, index) in data.form.FocusDanmu" :label="'关注答谢语 ' + (index + 1)">
          ...
          <el-button class="mt-2" @click="deleteFocusDanmu(index)">删除
          </el-button>
        </el-form-item>
      </el-tab-pane>
      <el-tab-pane label="定时弹幕" name="sixth">
        ...
        <el-form-item>
          <el-tag>Tips1: 定时弹幕corn表达式 格式为 秒 分 时 日 月 星期 其中秒是可选的
        ...
        <el-form-item>
          <el-tag>Tips4: 随机开关打开表示随机发送列表中的一条弹幕,
            关闭则是顺序发送</el-tag>
        </el-form-item>
        <el-form-item>
          <el-button type="primary" @click="addDanmulist">新增弹幕列表
          </el-button>
        </el-form-item>
        <div v-for="(items, index) in data.form.CronDanmuList">
          <el-form-item :label="'弹幕列表' + (index + 1)">
            <el-button type="danger" @click="deleteDanmulist(index)">删除
            </el-button>
          </el-form-item>
          ...
          <el-form-item>
            <el-button type="primary" @click="addDanmu(index)">新增弹幕
            </el-button>
          </el-form-item>
          ...
          <el-form-item :label="'弹幕' + (index2 + 1)" v-for="(items, index2) in data.form.CronDanmuList[index].Danmu">
            ...
            <el-button class="mt-2" @click="deleteDanmu(index, index2)" type="danger">删除
            </el-button>
          </el-form-item>
        </div>
      </el-tab-pane>
      <el-tab-pane label="欢迎黑名单-模糊匹配" name="seventh">
        ...
        <el-form-item v-for="(items, index) in data.form.WelcomeBlacklistWide" :label="'模糊关键词 ' + (index + 1)">
          <el-input v-model="data.form.WelcomeBlacklistWide[index]" />
          <el-button class="mt-2" @click="deleteWelcomeBlacklistWide(index)">删除
          </el-button>
        </el-form-item>
      </el-tab-pane>
      <el-tab-pane label="欢迎黑名单-精确匹配" name="eighth">
        ...
        <el-form-item v-for="(items, index) in data.form.WelcomeBlacklist" :label="'精确关键词 ' + (index + 1)">
          <el-input v-model="data.form.WelcomeBlacklist[index]" />
          <el-button class="mt-2" @click="deleteWelcomeBlacklist(index)">删除
          </el-button>
        </el-form-item>
      </el-tab-pane>
  ...
  <el-dialog v-model="data.keywordReplyDialogVisible" :title="data.keywordReplyDialogtitle" width="30%">
    <el-form-item label="关键词">
      <el-input v-model="data.keywordDialogData.keyword" :disabled="data.keywordReplyDialogAction" />
    </el-form-item>
